...
namespace Pyrra.Application.Tests.Desafios {
    public class TournamentChallengeServiceTests {
        ...
        private static (TournamentChallengeService service, FakeTournamentRepository tournaments,
            ...
            var service = new TournamentChallengeService(tournaments, challenges, categories, links, ownChallenges, submissions, teams, users, clock);
            return (service, tournaments, challenges, categories, links, ownChallenges, clock, submissions, teams, users);
        }
        ...
        public async Task GetCatalog_ComoDono_ListaComStatusDeVinculo() {
            var (service, _, challenges, categories, links, _, _, _, _, _) = Build();
        ...
        public async Task LinkCatalogChallenge_ComoDono_Vincula() {
            ...
            await service.LinkCatalogChallengeAsync(OwnerId, TournamentId, challenge.Id);
        ...
        public async Task LinkCatalogChallenge_ComoNaoDono_Lanca() {
            var (service, _, challenges, categories, _, _, _, _, _, _) = Build();
            var category = MakeCategory();
            categories.Categories.Add(category);
            var challenge = MakeChallenge(category.Id);
            challenges.Challenges.Add(challenge);

            await Assert.ThrowsAsync<NotFoundException>(() => service.LinkCatalogChallengeAsync(OutsiderId, TournamentId, challenge.Id));
        }
        ...
        public async Task LinkCatalogChallenge_DesafioInexistente_Lanca() {
            ...
            await Assert.ThrowsAsync<NotFoundException>(() => service.LinkCatalogChallengeAsync(OwnerId, TournamentId, Guid.NewGuid()));
        }
        ...
        public async Task CreateOwnChallenge_ComoDono_Cria() {
            var (service, _, _, _, _, ownChallenges, clock, _, _, _) = Build();

            var challenge = await service.CreateOwnChallengeAsync(OwnerId, TournamentId, "Corrida 10km", "Descrição", 30);
        ...
        public async Task CreateOwnChallenge_ComoNaoDono_Lanca() {
            ...
            await Assert.ThrowsAsync<NotFoundException>(() =>
                service.CreateOwnChallengeAsync(OutsiderId, TournamentId, "Título", null, 10));
        }
        ...
        public async Task CreateOwnChallenge_SemTitulo_Lanca() {
            ...
            await Assert.ThrowsAsync<InvalidChallengeException>(() =>
                service.CreateOwnChallengeAsync(OwnerId, TournamentId, "  ", null, 10));
        }
        ...
        public async Task CreateOwnChallenge_PontosZeroOuNegativo_Lanca() {
            ...
            await Assert.ThrowsAsync<InvalidChallengeException>(() =>
                service.CreateOwnChallengeAsync(OwnerId, TournamentId, "Título", null, 0));
        }

        [Fact]
        public async Task UpdateOwnChallenge_ComoDono_Atualiza() {
            var (service, _, _, _, _, ownChallenges, _, _, _, _) = Build();
            var challenge = await service.CreateOwnChallengeAsync(OwnerId, TournamentId, "Original", null, 10);

            var updated = await service.UpdateOwnChallengeAsync(OwnerId, TournamentId, challenge.Id, "Atualizado", "Nova descrição", 25);
        ...
        public async Task UpdateOwnChallenge_ComoNaoDono_Lanca() {
            ...
            await Assert.ThrowsAsync<NotFoundException>(() =>
                service.UpdateOwnChallengeAsync(OutsiderId, TournamentId, challenge.Id, "Atualizado", null, 20));
        }

        [Fact]
        public async Task UpdateOwnChallenge_DeOutroTorneio_Lanca() {
            var (service, tournaments, _, _, _, ownChallenges, _, _, _, _) = Build();
            ...
            await Assert.ThrowsAsync<NotFoundException>(() =>
                service.UpdateOwnChallengeAsync(OwnerId, otherTournamentId, challenge.Id, "Atualizado", null, 20));
        ...
        public async Task GetOwnChallenges_ListaOrdenadaPorTitulo() {
            var (service, _, _, _, _, _, _, _, _, _) = Build();
            await service.CreateOwnChallengeAsync(OwnerId, TournamentId, "Zebra", null, 10);
            await service.CreateOwnChallengeAsync(OwnerId, TournamentId, "Abelha", null, 10);
        ...
        public async Task GetPendingSubmissions_CruzaVariosTimesDoMesmoTorneio() {
            var (service, _, _, _, _, ownChallenges, clock, submissions, teams, users) = Build();
        ...
        public async Task GetPendingSubmissions_IgnoraDesafioDeTimeEDeOutroTorneio() {
            var (service, tournaments, _, _, _, ownChallenges, clock, submissions, teams, users) = Build();
            users.Users.Add(new Pyrra.Domain.Users.User { Id = SubmitterId, Name = "Membro", Email = "membro@x.com" });
            teams.Teams.Add(new Team { Id = Team1Id, Name = "Time 1", OwnerId = Guid.NewGuid(), MemberLimit = 10, InviteToken = "t1" });
            var otherTournamentId = Guid.NewGuid();
